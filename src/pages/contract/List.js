...
function mapStateToProps(state) {
  return {
    initContractList: state.contract.initContractList,
    listLoading: state.loading.effects['customer/Customer'],
    initState: state.contract.initState,
  };
}
...
class List extends Component {
  ...
  columns = [
    ...
    },
    {
      ...
      render:(text) => {
        return text === '0'? (<Tag color='red'>建设工程合同</Tag>):(<Tag color='green'>运输合同</Tag>)
      }
    },
    {
      title: '甲方单位',
  ...
  componentDidMount() {
    this.props.dispatch({
      type: 'contract/queryContractList',
      payload: {
        pageNum:0,
        pageSize:10,
      },
  ...
  //详情页面
  handleDetail(data){
    this.props.history.push({ pathname: '/contract/details', state: { id:data.id,data:data }})
  }
  //详细情况
  ...
  handleUpdate(data){
    this.props.history.push({ pathname: '/contract/Contract/add', state: { isAdd:0,data:data } })
  }
  ...
  render() {
    const { initContractList,listLoading } = this.props;
    return (
      <div>
        <div className={styles.tableListForm}>{this.renderForm()}</div>
        <div className="tr">
          <Button type="primary" className='add fl'><Link to={{pathname:'/contract/Contract/add',state:{isAdd:1}}}>新增</Link></Button>
        </div>
        <Table
          style={{marginTop:20}}
          columns={this.columns}
          dataSource={initContractList.content}
          loading={listLoading}
          rowKey="id"
          pagination={{  // 分页
            defaultCurrent: 1,
            pageSize:10,
            total: initContractList.totalElements,
