/* eslint-disable */
import React,{ Component } from "react";
import { Table,Divider,Button,Input,Modal,message,Avatar,Select,Checkbox, Row,Col,Tag,Icon,InputNumber,DatePicker,} from 'antd';
import { connect } from 'dva';
import Link from 'umi/link';
import styles from '../index.less';
import { Form } from 'antd/lib/index';
import moment from 'moment';

const FormItem = Form.Item;
const Search = Input.Search;
const confirm = Modal.confirm;
const Option = Select.Option;

function mapStateToProps(state) {
  return {
    initDangerList: state.dangerManage.initDangerList,
    listLoading: state.loading.effects['dangerManage/queryDangerList'],
    initState: state.dangerManage.initState,
    dangerType:state.dangerManage.dangerType,
  };
}

class List extends Component {
  state = {
    dangerLevel:'',
    dangerStatus:'',
    visible: false,
  }

  columns = [
    {
      title: '隐患描述',
      dataIndex: 'remark',
      render:(text,value)=>{
        return (<a onClick={this.handleDetail.bind(this,value)}>{text}</a>)
      }
    },
    {
      title: '隐患设施',
      dataIndex: 'equipmentName',
      render:(text,value)=>{
        return (<a onClick={this.handleEquipmentDetail.bind(this,value)}>{text}</a>)
      }
    },
    {
      title: '隐患位置',
      dataIndex: 'dangerAddress',
    },
    {
      title: '隐患级别',
      dataIndex: 'dangerLevelName',
      render:(text) => {
        return (<Tag color='red'>{text}</Tag>)
      }
    },
    {
      title: '隐患类型',
      dataIndex: 'dangerTypeName',
      render:(text) => {
        return (<Tag color='red'>{text}</Tag>)
      }
    },
    {
      title: '提交日期',
      dataIndex: 'createDate',
      defaultSortOrder: 'descend',
      sorter: (a, b) => a.createDate - b.createDate,
      render:(text) => {
        return <Tag color='red'>{moment(text).format('YYYY-MM-DD HH:mm:ss')}</Tag>
      }
    },
    {
      title: '隐患状态',
      dataIndex: 'dangerStatus',
      defaultSortOrder: 'descend',
      sorter: (a, b) => a.dangerStatus - b.dangerStatus,
      render:(text)=>{
        switch (text){
          case '0':
            return <Tag color='red'>待整改</Tag>
          case '1':
            return <Tag color='orange'>整改中</Tag>
          case '2':
            return <Tag color='blue'>待复查</Tag>
          case '3':
            return <Tag color='green'>已完成</Tag>
          default:
            return <span></span>
        }
      }
    },
    {
      title: '操作',
      width:210,
      render: (text, record) => (
        <span>
          <a onClick={this.handleDangerRepair.bind(this,record)}>指派</a>
          <Divider type="vertical" />
          <a onClick={this.handleDangerRepair.bind(this,record)}>整改</a>
        </span>
      ),
    }
  ];
  componentDidMount() {
    //从status里取
    this.props.dispatch({
      type: 'dangerManage/queryDangerList',
      payload: {
        dangerLevel:'',
        dangerStatus:this.state.dangerStatus,
        pageNum:'',
        pageSize:'',
        remark:'',
        equipmentName:'',
        dangerTypeName:'',
        createDate:'',
        dangerTypeId:''
      },
    });

    // 查询隐患类型列表
    this.props.dispatch({
      type: 'dangerManage/queryDangerType',
      payload: {
        typeId:'2',
      },
    });
  }
  handleSearch=(e)=>{
    e.preventDefault();
    this.props.form.validateFields((err, values) => {
      console.log(values)
      if (!err) {
        const filedValues = {
          ...values,
          'remark': typeof(values['remark']) == "undefined"?"":values['remark'] ,
          'equipmentName': typeof(values['equipmentName']) == 'undefined'?'':values['equipmentName'],
          'dangerTypeName': typeof(values['dangerTypeName']) == 'undefined'?'':values['dangerTypeName'],
          'createDate': typeof(values['createDate']) == 'undefined'?"":values['createDate'].format('YYYY-MM-DD'),
          'dangerStatus': typeof(values['dangerStatus']) == 'undefined'?'':values['dangerStatus'],
          'dangerTypeId': typeof(values['dangerTypeId']) == 'undefined'?'':values['dangerTypeId'],

        };
        console.log(filedValues)
        //从status里取
        this.props.dispatch({
          type: 'dangerManage/queryDangerList',
          payload: {
            dangerLevel:'',
            dangerStatus:filedValues.dangerStatus,
            pageNum:'',
            pageSize:'',
            remark:filedValues.remark,
            equipmentName:filedValues.equipmentName,
            dangerTypeName:filedValues.dangerTypeName,
            createDate:filedValues.createDate,
            dangerTypeId:filedValues.dangerTypeId
          },
        });
        console.log(this.props.initDangerList)
      }

    });

  }
  handleDangerRepair(data){
    if(data.dangerStatus != '0'){
      message.warning('非待整改隐患!');
    }else {
      this.props.history.push({
        pathname: '/dangerManage/dangerManage/dangerRepair',
        state: { id: data.id, data: data }
      })
    }
  }

  handleDangerReview(data){
    if(data.dangerStatus != '2'){
      message.warning('非待复查隐患!');
    }else {
      this.props.history.push({
        pathname: '/dangerManage/dangerManage/dangerReview',
        state: { id: data.id, data: data }
      })
    }
  }

  handleDetail(data){
    this.props.history.push({ pathname: '/dangerManage/details', state: { id:data.id,data:data }})
  }
  //设备查看
  handleEquipmentDetail(data){
    this.props.history.push({ pathname: '/deviceManagement/details', state: { id:data.equipmentId,data:data } })
  }

  handleDelete(data){
    const that = this;
    confirm({
      title: '确认删除该条数据',
      okText: '确认',
      okType: 'danger',
      cancelText: '取消',
      onOk(close) {
        that.props.dispatch({
          type: 'dangerManage/deleteDangerInfo',
          payload: data.id
        }).then(() => {
          if (that.props.initState.code === 0) {
            message.success('删除成功！');
            close();
            that.componentDidMount();
          } else {
            message.error('删除失败！请稍后重试');
            close();
          }
        })
      }
    })
  }
  changeSize = (page) => {
    this.props.dispatch({
      type: 'dangerManage/queryDangerList',
      payload: {
        dangerLevel:this.state.username,
        dangerStatus:this.state.dangerStatus,
        pageNum:page-1,
      },
    });
  }
  //重置
  handleFormReset = () => {
    const { form, dispatch } = this.props;
    form.resetFields();
    this.setState({
      formValues: {},
    });
    dispatch({
      type: 'rule/fetch',
      payload: {},
    });
  };

  renderSimpleForm() {
    const { getFieldDecorator } = this.props.form;
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="隐患描述">
              {getFieldDecorator('remark')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="隐患设备">
              {getFieldDecorator('equipmentName')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <span>
              <Button type="primary" htmlType="submit">
                查询
              </Button>
              <Button style={{ marginLeft: 8 }} onClick={this.handleFormReset}>
                重置
              </Button>
              <a style={{ marginLeft: 8 }} onClick={this.toggleForm}>
                展开 <Icon type="down" />
              </a>
            </span>
          </Col>
        </Row>
      </Form>
    );
  }

  renderAdvancedForm() {
    const { getFieldDecorator } = this.props.form;
    const { dangerType } = this.props;
    return (
      <Form onSubmit={this.handleSearch} layout="inline">
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="隐患描述">
              {getFieldDecorator('remark')(<Input placeholder="请输入" />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="隐患设备">
              {getFieldDecorator('equipmentName')(<Input style={{ width: '100%' }} />)}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="隐患状态">
              {getFieldDecorator('dangerStatus')(
                <Select placeholder="请选择" style={{ width: '100%' }}>
                  <Option value="0">待整改</Option>
                  <Option value="1">整改中</Option>
                  <Option value="2">待复查</Option>
                  <Option value="3">已完成</Option>
                </Select>
              )}
            </FormItem>
          </Col>
        </Row>
        <Row gutter={{ md: 8, lg: 24, xl: 48 }}>
          <Col md={8} sm={24}>
            <FormItem label="上报日期">
              {getFieldDecorator('createDate', {
                rules: [{ type: 'object'}]
              })(
                <DatePicker format="YYYY-MM-DD" style={{ width: '100%' }} placeholder="请输入上报日期" />
              )}
            </FormItem>
          </Col>
          <Col md={8} sm={24}>
            <FormItem label="隐患类型">
              {getFieldDecorator('dangerTypeId')(
                <Select placeholder="请选择" style={{ width: '100%' }}>
                  {
                    dangerType.map((item) => {
                      return (<Option key={item.id} value={item.id}>{item.name}</Option>)
                    })
                  }
                </Select>
              )}
            </FormItem>
          </Col>
        </Row>
        <div style={{ overflow: 'hidden' }}>
          <span style={{ float: 'right', marginBottom: 24 }}>
            <Button type="primary" htmlType="submit">
              查询
            </Button>
            <Button style={{ marginLeft: 8 }} onClick={this.handleFormReset}>
              重置
            </Button>
            <a style={{ marginLeft: 8 }} onClick={this.toggleForm}>
              收起 <Icon type="up" />
            </a>
          </span>
        </div>
      </Form>
    );
  }

  renderForm() {
    return this.state.expandForm ? this.renderAdvancedForm() : this.renderSimpleForm();
  }

  toggleForm = () => {
    this.setState({
      expandForm: !this.state.expandForm,
    });
  };

  render() {
    const { initDangerList,listLoading } = this.props;
    return (
      <div>
        <div className={styles.tableListForm}>{this.renderForm()}</div>
        <div className="tr">
          <Button type="primary" className='add fl'><Link to={{pathname:'/dangerManage/dangerManage/add',state:{isAdd:1}}}>新增</Link></Button>
        </div>
        <Table
          style={{marginTop:20}}
          columns={this.columns}
          dataSource={initDangerList.content}
          loading={listLoading}
          rowKey="id"
          pagination={{  // 分页
            defaultCurrent: 1,
            pageSize:10,
            total: initDangerList.totalElements,
            onChange:this.changeSize,
          }}
        />
      </div>
    );
  }
}

const AddForm = Form.create()(List);
export default connect(mapStateToProps)(AddForm);
